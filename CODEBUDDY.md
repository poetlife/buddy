...
个人向 CLI 辅助工具，基于 Rust + clap 实现。

## 文档索引

| 文档 | 路径 | 说明 |
|------|------|------|
| 可观测性总览 | [docs/observability.md](docs/observability.md) | 日志接入方式与规范 |
...
- **发版**:发版前必须阅读并遵循 [`docs/release.md`](docs/release.md)。不按此流程操作的发版视为无效。
...
- 避免：`utils.rs`、`helper.rs`、`common.rs`、`misc/`
- 推荐：`order_validator.rs`、`payment_gateway.rs`、`retry_policy.rs`

**单一职责**：一个模块只做一件事。模块开始"又做 A 又做 B"时，是拆分的信号，而不是继续往里加的理由。
...
- 模块 A 用方法 1 做了判断，模块 B 做**同样**判断时另写方法 2（即使"两行就搞定"）
...
2. 已登记 → 直接复用；需求不满足时改那一个��口，不得在调用侧绕开
